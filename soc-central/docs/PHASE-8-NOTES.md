...
## What shipped (`deploy/`)
- **Helm chart** `deploy/helm/soc-central/` — templates the product workloads (api ×2,
...
- **Identity** — `identity/realm-soc-central.json` (Keycloak realm: roles soc-admin /
...
## Verified (2026-06-02)
- `helm lint deploy/helm/soc-central` → **0 charts failed** (only the cosmetic "icon
  recommended" info).
- `helm template soc deploy/helm/soc-central` renders cleanly: 4 NetworkPolicies,
