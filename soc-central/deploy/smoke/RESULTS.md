...
helm lint   deploy/helm/soc-central            # 0 failed
helm template soc deploy/helm/soc-central      # 4 Deployment, 4 CronJob, 4 NetworkPolicy, 3 Service, 1 Ingress, 1 SA, 1 ConfigMap
helm install soc deploy/helm/soc-central -n soc-central --dry-run=server
...
`live-workloads.yaml` deploys the real `soc-central-api` + `soc-central-console` images
