...
def train(extra_X: np.ndarray | None = None, extra_y: np.ndarray | None = None,
    ...
    META_PATH.write_text(json.dumps({
        ...
        # The caveat travels WITH the number. meta.json is what gets read into write-ups
        # and screenshots, and an unqualified r2=0.94 there would be read as "94% accurate
        # at prioritising real vulnerabilities", which it is not.
        "metrics_scope": "synthetic holdout (same generator as training, different seed)",
        "metrics_caveat": (
            "Labels come from dataset._label = composite() + fixed interaction boosts + "
            "N(0,4) noise, i.e. they are generated by the formula under test. These "
            "metrics show the fit converged on a smooth synthetic target; they are NOT "
            "accuracy on real findings and must not be reported as such. Real evaluation "
            "needs analyst labels in analyst_feedback — see docs/METHODOLOGY.md §4.1."
        ),
        "n_samples": int(len(y)), "best_iteration": int(booster.best_iteration),
        "importance_gain": {k: round(v, 1) for k, v in gain.items()},
    }, indent=2))
    log.info("trained %s metrics=%s [synthetic self-referential holdout — NOT real-world "
             "accuracy, see metrics_caveat] -> %s", version, metrics, MODEL_PATH)
