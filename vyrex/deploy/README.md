...
```
deploy/
  helm/vyrex/      Helm chart for the PRODUCT workloads + security primitives
...
5. **Identity**: import `identity/realm-vyrex.json`, apply `identity/oauth2-proxy.yaml`.
6. **App**: `helm install soc deploy/helm/vyrex -n vyrex` (or hand it to ArgoCD
...
| Requirement | Where |
|-------------|-------|
| Helm charts | `helm/vyrex/` (lint-clean; renders 4 Deployments, 4 CronJobs, 4 NetworkPolicies, Ingress, …) |
...
```bash
docker run --rm -v "$PWD/deploy/helm:/charts" alpine/helm:3.16.3 lint /charts/vyrex
docker run --rm -v "$PWD/deploy/helm:/charts" alpine/helm:3.16.3 template soc /charts/vyrex
```
